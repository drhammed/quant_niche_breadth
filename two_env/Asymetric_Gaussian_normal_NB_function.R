...
generate_community.Asymetric.Gaussian.multi.x <- function(n.species,n.sites,n.x){
  ...
  repeat {
    ...
    # makes each species have different niches for any given x variable:
    #left_breadth <- matrix(runif(n.x * n.species,0.01,4),n.x,n.species)  
    #right_breadth <- matrix(runif(n.x * n.species,0.01,4),n.x,n.species)
    
    #make each species have different niches for any x variable and follows normal distribution
    left_breadth <- matrix(rnorm(n.x * n.species,0.01,0.8),n.x,n.species)
    right_breadth <- matrix(rnorm(n.x * n.species,0.01,0.8),n.x,n.species)
    #left_breadth <- matrix(rnorm(n.x * n.species,2.5,1.27),n.x,n.species)
    #right_breadth <- matrix(rnorm(n.x * n.species,2.5,1.27),n.x,n.species)
