...
generate_community.Asymetric.Gaussian.multi.x <- function(n.species,n.sites,n.x,shape,rate){
  ...
  repeat {
    ...
    # Generate different left and right breadths for each species and each environmental variable
    left_breadth <- matrix(rgamma(n.x * n.species, shape = shape, rate = rate), n.x, n.species)
    left_breadth <- pmax(left_breadth, 0.05)
    
    right_breadth <- matrix(rgamma(n.x * n.species, shape = shape, rate = rate), n.x, n.species)
    right_breadth <- pmax(right_breadth, 0.05)
