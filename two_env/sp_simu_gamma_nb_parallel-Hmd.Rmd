---
title: "sp_simu_gamma_nb_parallel"
...
gamma_Symmetric_Gaussian_function <- source("Symmetric_Gaussian_gamma_function.R")
gamma_AsymetricGaussian_functions <- source("AsymetricGaussian_gamma_functions.R")
...
result_df <- foreach(iteration = 1:n.reps, .packages=c("pacman", "ade4"), .export =
                       c("AllNicheBreadthFunctions_Hmd", "gamma_Symmetric_Gaussian_function",
                         "gamma_AsymetricGaussian_functions", "DavidZelenyThetaFunctions")) %dopar% {
  ...
  source("Symmetric_Gaussian_gamma_function.R")
  source("AsymetricGaussian_gamma_functions.R")
  #source("random_scenario_function.R")
...
# Parallelize the loop using foreach
result_df_asy <- foreach(iteration = 1:n.reps, .packages=c("pacman", "ade4"), .export =
                       c("AllNicheBreadthFunctions_Hmd", "gamma_Symmetric_Gaussian_function",
                         "gamma_AsymetricGaussian_functions", "DavidZelenyThetaFunctions")) %dopar% {
  ...
  simul.com <- generate_community.Asymetric.Gaussian.multi.x(n.species=30,n.sites=500,n.x=2,
                                                             shape=0.2,rate=0.5)
...
df_gamma <- Sym_mean_correlation_results %>% 
  inner_join(asy_mean_correlation_results, by = "metric")

write.csv(df_gamma, file = "df_gamma.csv")
...
#save.image("gamma_model_result_15Sept.RData")
