...
generate_abundances.Symmetric.Gaussian.multi.x <- function(x, optima, breadth, h) {
  ...
  for (i in 1:n.x) {
    Abundance.oracle[, i] <- rpois(n.sites, 30 * h[i] * sqrt(exp(-(x[, i] - optima[i])^2 / (2 * breadth^2))))
...
generate_community.Symmetric.Gaussian.multi.x <- function(n.species, n.sites, n.x,shape,rate) {
  ...
  repeat {
    ...
    breadth <- matrix(rgamma(n.x*n.species, shape=shape, rate=rate), n.x, n.species) #gamma distribution
    breadth <- pmax(breadth, 0.08)
