...
nr_hypervolume <- function(df_melted, nsamples = 1000) {
  ...
  sim.par <- tapply(1:nrow(df_melted), df_melted$species,
                    function(ii) niw.post(nsamples = nsamples, X = df_melted[ii, 2]))
...
nr_hypervolume_fun <- function(data, env_vars, nsamples = 1000) {
  ...
  # Select the species, presence-absence, and environmental variables
  df_melted <- df_melted[, c("species" , "PA", "env.1")]
  ...
  niw.post <- function(nsamples, X, lambda, kappa, Psi, nu) {
    ...
    # Add a small constant to Psi to avoid singularity issues when inverting the covariance matrix.
    #and to improve the numerical stability (regularization) of computations involving the covariance matrix.
  ...
  sim.par <- tapply(1:nrow(df_melted), df_melted$species, function(ii) {
    niw.post(nsamples = nsamples, X = df_melted[ii, 2], Psi = psi)
  })
...
hypervolume_blond <- function(df_melted) {
  ...
  for (sp in species_values) {
    ...
    sim_data <- df_melted[df_melted$species == sp, "env.1"]
...
estimate_nicheBreadth_Latents <- function(distribution_data, env_variables,nlv=5){
  n.species <- ncol(distribution_data)
  distribution_data.smaller <- eliminate_cols(distribution_data, min(colSums(distribution_data)), max(colSums(distribution_data)))
