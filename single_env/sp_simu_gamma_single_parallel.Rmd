...
AllNicheBreadthFunctions_Hmd <- source("AllNBFunctions_single.R")
Symmetric_Gaussian_function <- source("Single_Sym_Gau_gamma_Function.R")
AsymetricGaussian_functions <- source("Single_Asy_Gau_gamma_Function.R")
...
# number of cores
num_cores <- 8 #24 cores on lab server, change it accordingly when running on local machine
...
result_df <- foreach(iteration = 1:n.reps, .packages=c("pacman", "ade4"), .export = c("AllNicheBreadthFunctions_Hmd", "Symmetric_Gaussian_function", "AsymetricGaussian_functions", "DavidZelenyThetaFunctions")) %dopar% {
  
  # Load required packages
  pacman::p_load(data.table,ade4,tidyverse,cubelyr,tmap,mapview,exactextractr,lubridate,prism,e1071,ade4,vegan,dplyr,reshape2,ggpmisc,ggplot2,e1071,corrplot,nicheROVER,hypervolume,ggpubr,mFD,ecoCopula)

  # Load custom functions
   source("AllNBFunctions_single.R")
   source("Single_Sym_Gau_gamma_Function.R")
   source("Single_Asy_Gau_gamma_Function.R")
  ...
  # Generate the community with new niche_breadth values
    ...
    simul.com <- generate_community.Symmetric.Gaussian.multi.x(n.species=30,n.sites=500,n.x=1,
                                                             shape=0.2,rate=0.5)
  ...
  omi_result <- omi_params_fun(env_vars = env_vars, 
                               PA = sim.com)
...
result_df_asy <- foreach(iteration = 1:n.reps, .packages=c("pacman", "ade4"), 
                    .export = c("AllNicheBreadthFunctions_Hmd", "Symmetric_Gaussian_function", 
                    "AsymetricGaussian_functions", "DavidZelenyThetaFunctions")) %dopar% {
  ...
  simulated.com <- generate_community.Asymetric.Gaussian.multi.x(n.species=30,n.sites=500,n.x=1,
                                                           shape = 0.2,rate=0.5)
...
# Combine the results from all iterations using rbindlist
#result_df_asy <- rbindlist(result_df_asy)
...
#result_df_asy_gamma <- result_df_asy


#scale the data

result_df_asy[,3:12] <- scale(result_df_asy[,3:12])
...
#Asym_mean_correlation_results <- asy_mean_correlation_results
...
#save.image("single_gamma_model_result_Sept19.RData")
